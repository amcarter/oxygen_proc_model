...
calc_reach_prop = function(VPU, COMID, lat, long, CRS, quiet=FALSE){
  ...
  nhdR::nhd_plus_get(vpu = VPU, force_dl = TRUE)
  nhdR::nhd_plus_get(vpu= VPU, force_unzip = TRUE)
...
sites$reach_proportion = unlist(mapply(calc_reach_prop, sites$VPU, sites$COMID,
                                       sites$latitude, sites$longitude, WGS84, 
                                       quiet=TRUE))
...
#pick out the variables you want, then join them to your site data
streamcat_data = select(streamcat_data, COMID, ElevWs, PrecipWs, TminWs,
                        TmaxWs, TmeanWs, RunoffWs, matches('^Pct[a-zA-z]+2011Ws$'),
